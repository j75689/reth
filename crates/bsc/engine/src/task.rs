        ...
        tokio::spawn(async move {
            loop {
                ...
                if !pipeline_sync && (sealed_header.number - 1) > trusted_header.number {
                    ...
                    // check if the number of disconnected headers is equal to the difference between
                    if disconnected_headers.len() as u64 != (sealed_header.number - 1) - trusted_header.number {
                        continue;
                    }
